...
| Proof / signature / key authority | 8 | repo-proven for core, needs ops proof for KMS | Ed25519 verification, trusted fingerprint matching, strict Attestor-specific signing canonical JSON, SPKI-DER fingerprints, PKI trust-chain verification, PKI-bound certificate verification, and OPS-SWEEP-19/21 repo-proven DSSE + in-toto-shaped release-kernel evidence packs and release-policy-control-plane policy bundles using payload type `application/vnd.in-toto+json`, in-toto Statement v1, DSSE envelopes, SHA-256 bundle/evidence digests, and key fingerprint binding. OPS-SWEEP-18 maps the primitive signing surface and localizes lazy keyless CA fallback to OPS-141. | Runtime production signing still needs external KMS/HSM proof; keyless CA must fail closed in production-like runtime; canonicalization parity, HA-safe decision-log bootstrap, and policy mutation audit shared-store proof remain follow-up gaps before stronger runtime signing claims. | Remediate OPS-141, OPS-142, OPS-148, OPS-149, and OPS-161 before stronger limited-enforcement claims. |

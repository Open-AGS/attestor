...
| Admin API key blast radius | not fully elevated | open / needs live test | Single admin key remains high-value if compromised. | Scope, rotate, document, and test admin-key blast radius; prefer least privilege. |
...
| P0 | Admin route authorization deep audit | Admin routes are high-authority hosted attack surface. | Live shadow, limited enforcement, enterprise pilot | Route-by-route authZ/input validation tests. |

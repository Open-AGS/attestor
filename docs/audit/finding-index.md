...
| Admin API key blast radius | `partial / live-proof-only` | P1 | customer authority; operational boundedness; auditability | OPS-SWEEP-06 adds role-scoped admin credential support (`ATTESTOR_ADMIN_*_API_KEY`), route-level role allowlists, admin actor-role audit fields, admin auth rate limiting, and HTTP tests. Legacy `ATTESTOR_ADMIN_API_KEY` remains a superuser compatibility fallback. | Prove live role-scoped operator key deployment and legacy superuser-key rotation before stronger live/Phase 2 claims. |
| OPS-57 admin route single global bearer | `closed / live-proof-only` | P1 | customer authority; operational boundedness; auditability | `src/service/request-context.ts` accepts role-scoped admin keys; `src/service/http/routes/admin-routes.ts` enforces route role allowlists and records actor roles; `tests/service-admin-routes-http.test.ts` locks account-admin allow and read-key mutation denial. | Capture live proof that operators use role-scoped keys and that the legacy superuser key is rotated or tightly held. |

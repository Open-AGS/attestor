...
| B-028 secret redaction coverage | closed | repo-proven | `scripts/secret-safe-output.ts`, `tests/production-readiness-secret-safe-output.test.ts`, `scripts/check/check-public-artifacts-redaction.mjs`, and the Security Scan workflow close the provider-pattern and committed public-artifact scan gaps localized by OPS-SWEEP-16. | Scanner coverage is limited to committed public roots; live/operator artifacts need separate proof/review. |

...
- Baseline HEAD: `300a6cda236e6d85d858b3a6eae3ebb1f7262e5b`
...
| Token / replay / idempotency | 8 | partial-repo | Explicit token TTL policy, DPoP binding, replay/freshness primitives. | Multi-instance replay and introspection stores need live shared-store proof. | Run Redis/Postgres-backed replay and outage tests. |
...
| Cloud / ops readiness | 5 | not proven / needs ops proof | Docs are honest about non-claims; repo has production-readiness scaffolding, HA ops gates, PKI/TLS gates, observability remediation indexes, PITR/Redis recovery hardening, OPS-SWEEP-07 provider/profile hardening, OPS-SWEEP-08 webhook proof gates, OPS-SWEEP-09 account proof gates, and stage-aware live proof gates. | Live cloud/IAM/KMS/network/storage observability proof, live PITR/Redis recovery proof, KEDA scaler proof, webhook provider binding proof, account callback/audit-chain proof, logging redaction, and billing controls are not fully assessed. | Continue ops audit with remaining provider/live proof surfaces and capture live proof artifacts. |
...
| Admin API key blast radius | not fully elevated | partial / live-proof-only | OPS-SWEEP-06 adds role-scoped admin credentials, route role allowlists, actor-role audit fields, admin auth rate limiting, and HTTP tests; legacy `ATTESTOR_ADMIN_API_KEY` remains a superuser compatibility fallback. | Prove live role-scoped operator key deployment and rotate or tightly hold the legacy superuser key. |
...
| `ops/**` audit gap | partial | not proven / live-proof-only for remediated slices | OPS-SWEEP-01 through OPS-SWEEP-09 are indexed; profile/provider/webhook/account/live-proof capture has repo-side hardening, while live cloud and provider proof remains incomplete. | Continue ops audit and capture live proof before live shadow. |
...
| P1 | Admin route live role-key proof | Admin routes are high-authority hosted attack surface even after repo-side route hardening. | Limited enforcement, enterprise pilot; recommended before live shadow | Live role-scoped key deployment, legacy superuser-key rotation/holding proof, and admin rate-limit abuse probe. |
| P1 | Branch protection signatures and PR reviews | CI gates are stronger when author identity and review are enforced by platform policy. | Enterprise pilot; recommended before live shadow | GitHub protection proof and negative merge tests. |
...
| Day 2 | Branch protection proof | Close signature/review governance gap. | GitHub protection API checks | Protection evidence doc | Unsigned/self-merge negative proof. |

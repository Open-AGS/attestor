...
function detectMissingInputs(provider: Provider, tlsMode: string): string[] {
  ...
  if (env('ATTESTOR_PUBLIC_HOSTNAME')) {
    ...
    requireOne('ATTESTOR_STRIPE_PRICE_PILOT_WORKFLOW');
    requireOne('ATTESTOR_STRIPE_PRICE_STARTER_WORKFLOW');
    requireOne('ATTESTOR_STRIPE_PRICE_PRO_WORKFLOW');
    requireOne('ATTESTOR_STRIPE_OVERAGE_PRICE_STARTER_WORKFLOW');
    requireOne('ATTESTOR_STRIPE_OVERAGE_PRICE_PRO_WORKFLOW');

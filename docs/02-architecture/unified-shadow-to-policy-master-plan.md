...
Status: saved repository-side master list after Attestor unlock step 26. This
...
| Complete | 26 |
| Remaining | 0 |
| Current state | Steps 01-06 are complete on `origin/master`; Step 07 records the shared-store inventory; Step 08 adds the PostgreSQL-backed atomic retry/replay store slice; Step 09 adds the PostgreSQL-backed shared source-history and outbox primitive. Step 10 selects Anthropic Claude Messages API as the first non-OpenAI runtime adapter target. Step 11 implements the narrow Anthropic Messages API runtime slice with digest-only evidence, strict tool-schema tests, bounded runtime policy, and an opt-in external-live smoke probe while keeping live failover and production readiness unclaimed. Step 12 adds the production go/no-go packet, preserving the target-bound production boundary while closing the first unlock tracker. Step 13 records the [Target-System Compatibility Matrix](target-system-compatibility-matrix.md), keeping Salesforce, Microsoft, ServiceNow, workflow/iPaaS, data/IAM, spend/procurement, health/insurance, and crypto target families as adapters into one engine. Step 14 records the [Shadow Event Canonical Schema](shadow-event-canonical-schema.md), adding the versioned digest-only event envelope and the bridge from existing admission shadow events. Step 15 records the [Action Surface Graph](action-surface-graph.md), adding the tenant-bound graph of systems, actions, resources, route coverage, and missing proof links. Step 16 records the [Evidence State Model](evidence-state-model.md), making observed, inferred, missing, conflicting, stale, untrusted, approved, and enforceable evidence states explicit before policy candidate work. Step 17 records the [Policy Candidate PR Contract](policy-candidate-pr-contract.md), adding review-only candidate diffs with schema digest, source-event digest set, inferred fields, missing evidence, risk score, replay digest, questions, and approval state. Step 18 records the [Active Question Engine](active-question-engine.md), ranking the smallest high-impact human questions by risk reduction, event coverage, review-load delta, and uncertainty while keeping the output decision-support-only. Step 19 records the [Counterexample replay generator](counterexample-replay-generator.md), adding digest-bound negative fixtures for tenant mismatch, stale approval, missing evidence, bypass route, repeated action, prompt injection, tool poisoning, unsafe approval, and crypto transaction abuse while keeping replay local, synthetic, and non-mutating. Step 20 records the [Policy Twin backtest](policy-twin-backtest.md), adding digest-bound historical decision projection, counterexample outcome checks, false-admit risk counts, missed-evidence counts, review-load impact, no-go reasons, and no-traffic/no-mutation/no-auto-enforcement invariants. Step 21 records the [Review-by-exception inbox](review-by-exception-inbox.md), adding the candidate-level lane contract for failed-replay, blocked-by-evidence, needs-answer, ready-to-approve, and monitoring-only items with one item per candidate, default-visible human action items only, no noisy event-level inspection, and no auto-enforcement. Step 22 records the [Approval/dismiss feedback loop](approval-dismiss-feedback-loop.md), adding digest-bound reviewer feedback for approvals, dismissals, stricter-version requests, threshold edits, and rollback requests without mutating policy bundles, updating candidates in place, retraining models, or activating enforcement. Step 23 records the [Enterprise integration recipes](enterprise-integration-recipes.md), adding source-backed pre-side-effect placement recipes for Salesforce/Agentforce, Microsoft Copilot/Power Automate, ServiceNow, Workato, MuleSoft, n8n, Zapier MCP, Zendesk, Intercom, Snowflake, Databricks, Okta, Entra, and SailPoint without claiming native connector coverage, customer deployment, or production readiness. Step 24 records the [General Crypto Transaction Gate](general-crypto-transaction-gate.md), adding a digest-only gate for native transfers, ERC-20 transfer/approve, permit signing, swaps, bridges, Safe transaction proposals, ERC-4337 UserOperations, session-key grants, EIP-7702 delegated authorization, and x402 payments without making Attestor a wallet, custodian, exchange, broadcaster, chain analytics provider, or production crypto runtime. Step 25 records the [Domain consequence recipes](domain-consequence-recipes.md), adding source-backed digest-only placement recipes for Ramp, Brex, Coupa, SAP S/4HANA, Snowflake, Databricks, Okta, Entra, SailPoint, FHIR Subscriptions, CDS Hooks, Guidewire ClaimCenter, and Guidewire PolicyCenter without claiming native connector coverage, records-system/workflow-workspace ownership, customer deployment, clinical/insurance compliance, or production readiness. Step 26 records the [Pilot readiness packet](pilot-readiness-packet.md), adding `attestor.pilot-readiness-packet.v1` with shadow-entry and scoped-enforcement-entry gates for integration prerequisites, shadow observation, event quality, evidence coverage, active-question resolution, approval path, rollout mode, rollback, decision-log/audit trail, and non-claim acknowledgement without claiming live customer pilot execution or production readiness. |
...
| 21 | complete | Review-by-exception inbox | `attestor.review-by-exception-inbox.v1`, `src/consequence-admission/review-by-exception-inbox.ts`, `docs/02-architecture/review-by-exception-inbox.md`, `tests/review-by-exception-inbox.test.ts`, research ledger entry, package script, and tracker/doc links for failed-replay, blocked-by-evidence, needs-answer, ready-to-approve, and monitoring-only lanes with one item per candidate, default-visible human action items only, no noisy event-level inspection, no raw payload storage, and no-auto-enforcement invariants. | Do not create a noisy dashboard that requires humans to inspect every event. |
| 22 | complete | Approval/dismiss feedback loop | `attestor.approval-dismiss-feedback-loop.v1`, `src/consequence-admission/approval-dismiss-feedback-loop.ts`, `docs/02-architecture/approval-dismiss-feedback-loop.md`, `tests/approval-dismiss-feedback-loop.test.ts`, research ledger entry, package script, and tracker/doc links for digest-bound approvals, dismissals, stricter-version requests, threshold edits, and rollback requests with review-context binding, no policy-bundle mutation, no candidate mutation in place, no model retraining, and no-auto-enforcement invariants. | Do not silently retrain or alter enforcement from feedback without a new approved candidate. |
| 23 | complete | Enterprise integration recipes | `attestor.enterprise-integration-recipes.v1`, `src/consequence-admission/enterprise-integration-recipes.ts`, `docs/02-architecture/enterprise-integration-recipes.md`, `tests/enterprise-integration-recipes.test.ts`, research ledger entry, package script, and tracker/doc links for source-backed placement recipes across Salesforce/Agentforce, Microsoft Copilot/Power Automate, ServiceNow, Workato, MuleSoft, n8n, Zapier MCP, Zendesk, Intercom, Snowflake, Databricks, Okta, Entra, and SailPoint. | Do not claim native integration coverage, customer deployment, marketplace certification, or production readiness from recipes. |
| 24 | complete | General Crypto Transaction Gate | `attestor.general-crypto-transaction-gate.v1`, `src/consequence-admission/general-crypto-transaction-gate.ts`, `docs/02-architecture/general-crypto-transaction-gate.md`, `tests/general-crypto-transaction-gate.test.ts`, research ledger entry, package script, and tracker/doc links for digest-only native transfer, ERC-20 transfer/approve, permit signing, swap, bridge, Safe transaction proposal, UserOperation, session-key grant, EIP-7702 delegated authorization, and x402 payment gate decisions. | Do not make Attestor a wallet, custodian, exchange, chain analytics provider, or transaction broadcaster. |
| 25 | complete | Spend, procurement, data, IAM, health, and insurance recipes | `attestor.domain-consequence-recipes.v1`, `src/consequence-admission/domain-consequence-recipes.ts`, `docs/02-architecture/domain-consequence-recipes.md`, `tests/domain-consequence-recipes.test.ts`, research ledger entry, package script, and tracker/doc links for source-backed placement recipes across Ramp, Brex, Coupa, SAP S/4HANA, Snowflake, Databricks, Okta, Entra, SailPoint, FHIR Subscriptions, CDS Hooks, Guidewire ClaimCenter, and Guidewire PolicyCenter. | Do not widen into a general workflow workspace, records system, EHR, insurance core system, or production-ready native connector. |
| 26 | complete | Pilot readiness packet | `attestor.pilot-readiness-packet.v1`, `src/consequence-admission/pilot-readiness-packet.ts`, `docs/02-architecture/pilot-readiness-packet.md`, `tests/pilot-readiness-packet.test.ts`, research ledger entry, package script, and tracker/doc links for shadow-entry and scoped-enforcement-entry pilot packets covering integration prerequisites, shadow duration, event quality, evidence coverage, active questions, approval path, rollout mode, rollback, decision-log/audit trail, and non-claim acknowledgement. | Do not call a pilot production-ready without live target probes, customer approval, and receipt evidence. |

## Implementation Order

The unified Shadow-to-Policy master sequence is complete repository-side:

```text
01 -> 26 complete
```

Future work should start from a new scoped tracker when a customer-specific
pilot, native connector, live deployment, or production promotion target is
defined:
...
- Human workload minimization, feedback, and triage patterns: [Microsoft Human-AI Interaction Guidelines](https://www.microsoft.com/en-us/research/blog/guidelines-for-human-ai-interaction-design/), [Microsoft Human-AI Experience Toolkit](https://www.microsoft.com/en-us/haxtoolkit/), [Google People + AI Guidebook](https://pair.withgoogle.com/guidebook/), [GitHub code scanning alert triage](https://docs.github.com/en/code-security/how-tos/manage-security-alerts/manage-code-scanning-alerts/resolving-code-scanning-alerts), and [Google Cloud Recommender API state metadata](https://cloud.google.com/recommender/docs/use-api).
- Pilot readiness, review-before-apply, and protected rollout posture: [Terraform plan](https://developer.hashicorp.com/terraform/cli/commands/plan), [Kubernetes dry-run](https://kubernetes.io/docs/reference/using-api/api-concepts/#dry-run), [GitHub deployment environments](https://docs.github.com/en/actions/reference/workflows-and-actions/deployments-and-environments), [NIST SP 800-218](https://csrc.nist.gov/pubs/sp/800/218/final), [Microsoft HAX Toolkit](https://www.microsoft.com/en-us/haxtoolkit/), and [Google People + AI Guidebook](https://pair.withgoogle.com/guidebook-v2/).
- Workflow/action integration surfaces: [Salesforce Agentforce actions](https://developer.salesforce.com/docs/ai/agentforce/guide/get-started-actions.html), [Microsoft Copilot Studio tools](https://learn.microsoft.com/en-us/microsoft-copilot-studio/add-tools-custom-agent), [ServiceNow IntegrationHub](https://www.servicenow.com/docs/r/integrate-applications/integration-hub/integrationhub.html), [Workato business approvals](https://docs.workato.com/en/agentic/agent-studio/business-approvals.html), [MuleSoft Agentforce API actions](https://docs.mulesoft.com/anypoint-code-builder/ai-enabling-api-project-topic-center), [n8n human-in-the-loop tools](https://docs.n8n.io/advanced-ai/human-in-the-loop-tools/), [Zapier MCP](https://docs.zapier.com/mcp/home), [Zendesk AI Agents API](https://developer.zendesk.com/api-reference/ai-agents/introduction/), and [Intercom Conversations API](https://developers.intercom.com/docs/references/2.10/rest-api/api.intercom.io/conversations/manageconversation).
- Agentic orchestration and human-in-the-loop patterns: [Camunda agentic orchestration](https://docs.camunda.io/docs/components/agentic-orchestration/agentic-orchestration-overview/), [Workato business approvals](https://docs.workato.com/en/agentic/agent-studio/business-approvals.html), and [n8n human-in-the-loop tools](https://docs.n8n.io/advanced-ai/human-in-the-loop-tools/).
...
- Data/IAM/spend/health/insurance target-system surfaces: [Ramp API documentation](https://docs.ramp.com/llms-api.txt), [Brex Expenses API](https://developer.brex.com/openapi/expenses_api/), [Coupa approvals API](https://docs.coupa.com/en/developer-documentation/the-coupa-core-api/resources/transactional-resources/approvals-api-approvals), [SAP S/4HANA purchase order API](https://help.sap.com/docs/SAP_S4HANA_CLOUD/bb9f1469daf04bd894ab2167f8132a1a/fd5b4bdc2e7147429cb2de044ccbd890.html), [Snowflake Cortex Agents REST API](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents-rest-api), [Databricks AI agent tools](https://docs.databricks.com/aws/en/generative-ai/agent-framework/agent-tool), [Okta Workflows Okta connector](https://help.okta.com/wf/en-us/Content/Topics/Workflows/connector-reference/okta/okta.htm), [Microsoft Entra ID Governance Lifecycle Workflows extensibility](https://learn.microsoft.com/en-us/entra/id-governance/lifecycle-workflow-extensibility), [SailPoint workflow triggers](https://documentation.sailpoint.com/saas/help/workflows/workflow-triggers.html), [HL7 FHIR Subscriptions](https://hl7.org/fhir/subscriptions.html), [CDS Hooks specification](https://cds-hooks.org/specification/current/), [Guidewire ClaimCenter Cloud API](https://docs.guidewire.com/cloud/cc/202603/apiref/), and [Guidewire PolicyCenter Cloud API](https://docs.guidewire.com/cloud/pc/202603/apiref/).
...
- General crypto transaction standards and risks: [ERC-20 / EIP-20](https://eips.ethereum.org/EIPS/eip-20), [EIP-712 typed data](https://eips.ethereum.org/EIPS/eip-712), [EIP-2612 permit](https://eips.ethereum.org/EIPS/eip-2612), [ERC-4337 account abstraction](https://eips.ethereum.org/EIPS/eip-4337), [ERC-7562 account abstraction validation rules](https://eips.ethereum.org/EIPS/eip-7562), [EIP-5792 wallet calls](https://eips.ethereum.org/EIPS/eip-5792), [ERC-7715 wallet permissions](https://eips.ethereum.org/EIPS/eip-7715), [EIP-7702 delegated EOA authorization](https://eips.ethereum.org/EIPS/eip-7702), [ERC-7683 cross-chain intents](https://eips.ethereum.org/EIPS/eip-7683), [Safe Smart Account Guards](https://docs.safe.global/advanced/smart-account-guards), [x402 docs](https://docs.x402.org/), and [Coinbase CDP x402 overview](https://docs.cdp.coinbase.com/x402/welcome).

...
| F4-LLM03-A agentic supply-chain coverage gap / single LLM provider | `partial` | PR #297 added `agentic-supply-chain-guard`; `docs/audit/f2-llm-provider-supply-chain-validation.md`; `src/api/openai.ts` and `src/api/anthropic.ts` remain separate optional live-model wrappers | OWASP LLM03 risk is valid. Agentic supply-chain guard covers repo-side component evidence; Attestor-owned OpenAI and Anthropic wrappers are now wired repository-side, but live failover execution, customer provider approval, and production runtime evidence remain incomplete. |
...
| F4-D Attestor-owned OpenAI usage / budget / prompt leakage scope | `partial` | `docs/audit/f2-llm-provider-supply-chain-validation.md`; `src/api/openai.ts`; `src/api/anthropic.ts`; `scripts/probe/probe-openai-live-smoke.ts`; `scripts/probe/probe-anthropic-live-smoke.ts`; `src/api/llm-provider-registry.ts`; `src/financial/cli.ts`; `src/financial/cli/live-scenario.ts`; evaluation packet docs | Current caller is optional financial CLI live-model proof path. No hosted consequence-admission route uses it. Provider registry, compatible failover route gating, route-readiness evidence gating, digest-only proof-context binding, OpenAI and Anthropic timeout/output-token/retry policy, provider response storage minimization, and opt-in OpenAI/Anthropic reasoning live smoke proof now exist; live failover, OpenAI vision smoke proof, Vertex AI, and Azure OpenAI remain incomplete before broader runtime-provider claims. |
...
| F5-NEW-1 exported `setKeylessCa` runtime injection | `fixed` | F5 Keyless CA Injection Boundary Validation (`docs/audit/f5-keyless-ca-injection-boundary-validation.md`); `test:f5-keyless-ca-injection-boundary-validation` | The generic setter no longer exists; keyless signer internals remain outside package `exports`. |
...
| F6-T9 plaintext env API keys in memory | `fixed` | F6 Tenant Key Cache Hardening (`docs/audit/f6-tenant-blast-radius-validation.md`); `test:f6-tenant-key-cache-hardening`; stores env tenant keys by `tenant.api-key` lookup hash and stores only a secret-derived env config digest for reload detection. | No remaining repository action for this scoped finding. |
...
| F8-R3 health body diagnostic richness | `fixed` | F8 Operational Resilience Validation; `tests/service-core-routes.test.ts`; `test:f8-operational-resilience-validation`; `/api/v1/health` remains process-liveness HTTP 200 and privacy-minimized, while `/api/v1/ready` is the traffic gate. Internal diagnostics remain in runtime/rehearsal evidence, not public probe bodies. | No remaining repository action for this scoped finding. |
...
| F8-R12 webhook signature route proof | `fixed` | F8 Operational Resilience Validation (`docs/audit/f8-operational-resilience-validation.md`); `test:stripe-webhook-support-hardening`; `test:service-stripe-webhook-service`; `test:service-email-webhook-service`; Stripe, SendGrid, and Mailgun service tests reject missing or invalid signatures before mutation; health contract requires signed webhook ingress. | No remaining repository action for this scoped finding. |
...
| F9-C3 data-residency / regional-pinning posture missing | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `data-residency.md`. | Live regional pinning and transfer-mechanism proof remain deployment/legal work. |
| F9-C4 retention/disposal policy missing | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `retention-policy.md`; release retention classes; replay ledger retention settings. | Customer legal retention schedule and WORM/SIEM retention remain external. |
| F9-C5 segregation-of-duties policy missing | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `segregation-of-duties.md`; F7 high-risk two-person activation validation; break-glass hardening. | Named role assignments and access-review samples remain organizational evidence. |
| F9-C6 vendor / third-party provider risk doc missing | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `third-party-providers.md`; agentic supply-chain guard; F2/F4 LLM provider supply-chain validation. | Vendor due diligence, DPA, and approval records remain organizational evidence. |
| F9-C7 BC/DR policy doc RTO/RPO posture unclear | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `backup-restore-dr.md` records RPO / RTO guidance and current DR boundary. | Live restore drills and managed failover remain deployment evidence. |
| F9-C8 accessibility / AI bias posture missing | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `ai-accessibility-bias-boundary.md`; ISO/IEC 42001 mapping. | Upstream model fairness, dataset, UI accessibility, and legal assessment remain customer-owned. |
| F9-C9 security-testing / pentest posture undocumented | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `security-testing.md`; package runner; audit remediation tracker. | Independent pentest reports and retest records remain external evidence. |
| F9-C10 cryptography / key-management policy doc missing | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `cryptography-policy.md`; F5 signing validations; signing verification docs. | External KMS/HSM custody and customer key-management program remain external. |
| F9-C11 privacy notice / data-flow template missing | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `privacy-notice-template.md`; data minimization policy. | Legal basis, DPA, data-subject process, and customer notice approval remain external. |
| F9-C12 shared-responsibility model implicit | `fixed` | F9 Compliance Gap Validation (`docs/audit/f9-compliance-gap-validation.md`); `test:f9-compliance-gap-validation`; `shared-responsibility-matrix.md`; compliance evidence boundary. | Contractual allocation and customer control owner assignments remain external. |
...
| F10-E3 break-glass rollout lacks distinct gate | `fixed` | F7 Break-Glass Hardening Validation (`docs/audit/f7-break-glass-hardening-validation.md`); `test:f7-break-glass-hardening-validation`; secondary approver, expiry, justification, reconciliation. | Live operator evidence remains external. |
...
| F10-E9 exported `resetKeylessCa` | `fixed` | F10 Escape Hatch Validation (`docs/audit/f10-escape-hatch-validation.md`); `test:f10-escape-hatch-validation`; `resetKeylessCaForTesting(reason)` replaces generic reset export; package exports do not expose signing internals. | Internal test helper remains for isolated tests. |
| F10-E10 degraded-mode TTL escape | `fixed` | F10 Escape Hatch Validation (`docs/audit/f10-escape-hatch-validation.md`); F8 Operational Resilience Validation (`docs/audit/f8-operational-resilience-validation.md`); `test:f10-escape-hatch-validation`; `test:f8-operational-resilience-validation`; `createDegradedModeGrant` max TTL enforcement. | No remaining repository action for this scoped finding. |
...
| ID | Current status | Evidence / overlap | Remaining action |
|---|---|---|---|
| F11-SC-1 container base images use floating tags | `fixed` | `Dockerfile` was already digest-pinned; HA/DR compose images are now digest-pinned; `scripts/check/check-supply-chain-baseline.mjs`; `test:f11-supply-chain-depth-validation`. | No remaining repository action for this scoped finding. Image refresh automation remains operational work. |
| F11-SC-2 observability stack uses `:latest` tags | `fixed` | F11 Supply Chain Depth Validation (`docs/audit/f11-supply-chain-depth-validation.md`); `test:f11-supply-chain-depth-validation`; `docker-compose.observability.yml`; Kubernetes observability deployment and Grafana Alloy patch; supply-chain baseline image checks. | No remaining repository action for this scoped finding. |
| F11-SC-3 high-trust npm dependency caret pinning | `fixed` | F11 Supply Chain Depth Validation (`docs/audit/f11-supply-chain-depth-validation.md`); `test:f11-supply-chain-depth-validation`; critical runtime dependencies are exact-pinned in `package.json` and root `package-lock.json`; supply-chain baseline checks the critical list. | Dev/noncritical dependency ranges remain lockfile-governed. |
| F11-SC-4 single OpenAI provider / provider registry contract | `partial` | `src/api/openai.ts` and `src/api/anthropic.ts` now record provider-returned model observations/proof context, apply timeout/output-token runtime policy, and expose opt-in reasoning live smoke probes; `src/api/llm-provider-registry.ts` records provider inventory and fail-closed route evaluation with compatible fallback gating. | Live failover execution, OpenAI vision smoke proof, customer provider approval, Vertex AI, and Azure OpenAI remain future work before resilience claims. |
...
| F11-SC-8 webhook ingress signature spot-check | `fixed` | F11 Supply Chain Depth Validation (`docs/audit/f11-supply-chain-depth-validation.md`); F8 Operational Resilience Validation (`docs/audit/f8-operational-resilience-validation.md`); `test:f11-supply-chain-depth-validation`; `test:stripe-webhook-support-hardening`; `test:service-email-webhook-service`; F8 webhook service tests reject missing/invalid Stripe, SendGrid, and Mailgun signatures before mutation. | No remaining repository action for this scoped finding. |
...
| F11-SC-10 connector/plugin component criticality | `fixed` | F11 Supply Chain Depth Validation (`docs/audit/f11-supply-chain-depth-validation.md`); `test:f11-supply-chain-depth-validation`; critical connector/provider libraries are exact-pinned; agentic supply-chain guard covers connector/plugin kinds and criticality. | No remaining repository action for this scoped package-level finding. |
...
| F11-SC-12 release-provenance token boundary | `fixed` | F11 Supply Chain Depth Validation (`docs/audit/f11-supply-chain-depth-validation.md`); `test:f11-supply-chain-depth-validation`; `release-provenance.yml` keeps `attestations: write` and `id-token: write` isolated to release provenance, with evaluation tag trigger plus explicit manual dispatch. | No remaining repository action for this scoped finding. |

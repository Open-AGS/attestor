...
| O04 demo CLI | `scripts/demo/demo-golden-operational-execution.ts` renders the Markdown-first Operational Execution demo and supports `--json` plus bounded `--scenario fixtures/golden-operational-execution-reviewer-sandbox.example.json`. | repo-proven once merged |

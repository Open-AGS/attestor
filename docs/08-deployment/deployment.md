...
| `ATTESTOR_OPENAI_LIVE_SMOKE_PROOF_DIGEST` | Production-like OpenAI reasoning calls only | None | Digest emitted by `npm run probe:openai-live-smoke`. Production-like OpenAI reasoning calls fail closed when this proof digest is absent, stale, malformed, or mismatched |
| `ATTESTOR_OPENAI_LIVE_SMOKE_PROOF_CHECKED_AT` | Production-like OpenAI reasoning calls only | None | ISO timestamp emitted by the OpenAI live smoke probe |
| `ATTESTOR_OPENAI_LIVE_SMOKE_PROOF_MODEL` | Production-like OpenAI reasoning calls only | None | Model id emitted by the OpenAI live smoke probe; must match the configured reasoning model |
| `ATTESTOR_OPENAI_LIVE_SMOKE_PROOF_PURPOSE` | Production-like OpenAI reasoning calls only | None | Probe purpose emitted by the OpenAI live smoke probe; currently `reasoning` only |
| `ATTESTOR_OPENAI_LIVE_SMOKE_PROOF_MAX_AGE_MINUTES` | No | `1440` | Maximum accepted age for OpenAI live smoke proof env evidence. Invalid values fail closed before a production-like OpenAI call |
| `ATTESTOR_ANTHROPIC_TIMEOUT_MS` | No | `120000` | Optional Anthropic Messages wrapper per-request timeout in milliseconds. Invalid values fail closed before a live provider call |
| `ATTESTOR_ANTHROPIC_MAX_ATTEMPTS` | No | `2` | Optional Anthropic wrapper retry budget. Hidden transport retries are disabled; this bounded wrapper retry loop is the only retry budget |
| `ATTESTOR_ANTHROPIC_RETRY_INITIAL_DELAY_MS` | No | `1000` | Initial jittered retry delay for Anthropic wrapper calls |
| `ATTESTOR_ANTHROPIC_RETRY_MAX_DELAY_MS` | No | `8000` | Maximum jittered retry delay for Anthropic wrapper calls |
| `ATTESTOR_ANTHROPIC_REASONING_MAX_OUTPUT_TOKENS` | No | `16000` | Maximum output-token budget for Anthropic reasoning and strict-tool calls. A call-requested `maxTokens` above this budget fails closed |
| `ATTESTOR_ANTHROPIC_LIVE_SMOKE_PROOF_DIGEST` | Production-like Anthropic reasoning calls only | None | Digest emitted by `npm run probe:anthropic-live-smoke`. Production-like Anthropic reasoning calls fail closed when this proof digest is absent, stale, malformed, or mismatched |
| `ATTESTOR_ANTHROPIC_LIVE_SMOKE_PROOF_CHECKED_AT` | Production-like Anthropic reasoning calls only | None | ISO timestamp emitted by the Anthropic live smoke probe |
| `ATTESTOR_ANTHROPIC_LIVE_SMOKE_PROOF_MODEL` | Production-like Anthropic reasoning calls only | None | Model id emitted by the Anthropic live smoke probe; must match the configured reasoning model |
| `ATTESTOR_ANTHROPIC_LIVE_SMOKE_PROOF_PURPOSE` | Production-like Anthropic reasoning calls only | None | Probe purpose emitted by the Anthropic live smoke probe; currently `reasoning` only |
| `ATTESTOR_ANTHROPIC_LIVE_SMOKE_PROOF_MAX_AGE_MINUTES` | No | `1440` | Maximum accepted age for Anthropic live smoke proof env evidence. Invalid values fail closed before a production-like Anthropic call |
...
| `ATTESTOR_RELEASE_SIGNING_PROVIDER` | No | Derived from PKI mode | Release signer provider declaration. Supported values are `runtime-ephemeral`, `file-pem`, and `external-kms`. `external-kms` intentionally fails closed until runtime release-token issuance is wired to an external KMS/HSM signer, so operators cannot accidentally declare KMS while the runtime signs with local PEM material |
...
| `ATTESTOR_EXTERNAL_KMS_PROVIDER` | No | None | External signer adapter selector for proof probes. The first supported value is `gcp-kms`; it does not by itself activate runtime release-token issuance |
| `ATTESTOR_GCP_KMS_KEY_VERSION_NAME` | No | None | Full Google Cloud KMS cryptoKeyVersion resource name used by the GCP KMS release signer probe |
| `ATTESTOR_GCP_KMS_KEY_ID` | No | None | Opaque release key id recorded in signer proof descriptors. Do not include raw tenant ids |
| `ATTESTOR_GCP_KMS_PUBLIC_KEY_REF` | No | None | Non-secret public verification key reference used to bind GCP KMS probe evidence |
...
| `ATTESTOR_PIPELINE_IDEMPOTENCY_ENCRYPTION_KEY` | Production-like: yes for `/api/v1/pipeline/run-async` and when `/api/v1/pipeline/run` clients use `Idempotency-Key`; local/dev: optional for keyless `/run` | None | Dedicated secret for encrypting pipeline idempotency replay responses at rest. Production-like runtimes reject the local admin-key fallback; routes fail before side effects if this is missing and an `Idempotency-Key` is supplied. |
| `ATTESTOR_PIPELINE_IDEMPOTENCY_STORE_PATH` | No | `.attestor/pipeline-idempotency.json` | File-backed encrypted retry/replay store for pipeline `/run` and `/run-async` idempotency when `ATTESTOR_CONTROL_PLANE_PG_URL` is not configured |
| `ATTESTOR_PIPELINE_IDEMPOTENCY_TTL_HOURS` | No | `24` | Replay retention window for pipeline idempotency records |
...
- Pipeline idempotency first slice: `/api/v1/pipeline/run-async` requires `Idempotency-Key` before quota, rate-limit, or queue work; `/api/v1/pipeline/run` consumes the header when clients supply it before quota, rate-limit, connector, signing, or queue work. Matching retries replay the stored response and payload conflicts return 409. Replay records are encrypted at rest in `.attestor/pipeline-idempotency.json` by default and move onto the shared PostgreSQL control-plane when `ATTESTOR_CONTROL_PLANE_PG_URL` is configured. Production-like runtimes require `ATTESTOR_PIPELINE_IDEMPOTENCY_ENCRYPTION_KEY` when clients use the header.

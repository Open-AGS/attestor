...
- use the [action surface graph](action-surface-graph.md) before building the evidence state model so tenant-bound systems, tools, resources, route coverage, and missing proof links are explicit before candidate generation

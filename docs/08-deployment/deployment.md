...
| `ATTESTOR_PIPELINE_IDEMPOTENCY_ENCRYPTION_KEY` | Production-like: yes when clients use `Idempotency-Key`; local/dev: optional | None | Dedicated secret for encrypting pipeline idempotency replay responses at rest. Production-like runtimes reject the local admin-key fallback; routes fail before side effects if this is missing and an `Idempotency-Key` is supplied. |
...
- Pipeline idempotency first slice: `/api/v1/pipeline/run` and `/api/v1/pipeline/run-async` consume `Idempotency-Key` before quota, rate-limit, connector, signing, or queue work. Matching retries replay the stored response and payload conflicts return 409. Replay records are encrypted at rest in `.attestor/pipeline-idempotency.json` by default and move onto the shared PostgreSQL control-plane when `ATTESTOR_CONTROL_PLANE_PG_URL` is configured. Production-like runtimes require `ATTESTOR_PIPELINE_IDEMPOTENCY_ENCRYPTION_KEY` when clients use the header.

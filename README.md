...
The [crypto intelligence buildout](docs/02-architecture/crypto-intelligence-buildout.md) adds the same summary discipline to programmable-money paths: risk signals, policy gaps, adapter readiness, operator-supplied risk inputs, missing evidence, aggregate performance budgets, and digest-first proof links without raw wallet/payment payload drilldown or native screening claims.

...
function main(): void {
  ...
  ok(!grafanaCloudKustomization.includes('secret-template.yaml'), 'Kubernetes observability bundle: Grafana Cloud overlay does not apply placeholder secret template as a resource');
  ok(grafanaCloudReadme.includes('Grafana Cloud OTLP') && grafanaCloudReadme.includes('basicauth') && grafanaCloudReadme.includes('Do not apply `secret-template.yaml` directly'), 'Kubernetes observability bundle: Grafana Cloud overlay documents authenticator-based OTLP wiring and placeholder-secret boundary');
  ...
  ok(!grafanaAlloyKustomization.includes('secret-template.yaml'), 'Kubernetes observability bundle: Grafana Alloy overlay does not apply placeholder secret template as a resource');
  ok(grafanaAlloyReadme.includes('Grafana-supported') && grafanaAlloyReadme.includes('bin/otelcol') && grafanaAlloyReadme.includes('Do not apply `secret-template.yaml` directly'), 'Kubernetes observability bundle: Grafana Alloy overlay documents the supported OTel Engine runtime and placeholder-secret boundary');

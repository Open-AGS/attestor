...
import { RESEARCH_PROVENANCE_LEDGER_AUDIT_REMEDIATION_ANCHORS } from './research-provenance-ledger-fixtures.js';
...
function testLedgerIndexesAuditRemediationClosure(): void {
  ...
  for (const { expected, message } of RESEARCH_PROVENANCE_LEDGER_AUDIT_REMEDIATION_ANCHORS) {
    includes(content, expected, message);
  }
}

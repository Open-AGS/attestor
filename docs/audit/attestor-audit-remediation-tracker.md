...
| F4-D Attestor-owned OpenAI usage / budget / prompt leakage scope | `partial` | `docs/audit/f2-llm-provider-supply-chain-validation.md`; `src/api/openai.ts`; `src/api/llm-provider-registry.ts`; `src/financial/cli.ts`; evaluation packet docs | Current caller is optional financial CLI live-model proof path. No hosted consequence-admission route uses it. Provider registry, digest-only proof-context binding, OpenAI timeout enforcement, output-token budget enforcement, provider `store: false`, and SDK hidden-retry disablement now exist; live failover and provider smoke proof remain incomplete before broader runtime-provider claims. |
...
| F11-SC-4 single OpenAI provider / provider registry contract | `partial` | `src/api/openai.ts` still uses OpenAI directly, but now records provider-returned model observations and drift warnings, applies timeout/output-token runtime policy, and disables provider-side response storage; `src/api/llm-provider-registry.ts` records provider inventory and fail-closed route evaluation. | Multi-provider live client selection, failover, and smoke proof remain future work before resilience claims. |

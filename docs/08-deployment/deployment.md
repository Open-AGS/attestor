...
| `ATTESTOR_PIPELINE_IDEMPOTENCY_ENCRYPTION_KEY` | Production-like: yes for `/api/v1/pipeline/run` and `/api/v1/pipeline/run-async`; local/dev: optional only when the local admin-key fallback is available | None | Dedicated secret for encrypting pipeline idempotency replay responses at rest. Production-like runtimes reject the local admin-key fallback; pipeline routes fail before side effects if this is missing. |
...
- Pipeline idempotency first slice: `/api/v1/pipeline/run` and `/api/v1/pipeline/run-async` require `Idempotency-Key` before quota, rate-limit, connector, signing, queue, or pipeline work. Matching retries replay the stored response and payload conflicts return 409. Replay records are encrypted at rest in `.attestor/pipeline-idempotency.json` by default and move onto the shared PostgreSQL control-plane when `ATTESTOR_CONTROL_PLANE_PG_URL` is configured. Production-like runtimes require `ATTESTOR_PIPELINE_IDEMPOTENCY_ENCRYPTION_KEY`.

...
const LIVE_PROOF_FLAGS = Object.freeze([
  ...
  { name: 'ATTESTOR_SHADOW_MUTATION_RATE_LIMIT_PROOF', minStage: 'live-shadow', description: 'Shadow POST/PATCH mutation abuse probes return 429 per tenant and route while separate mutation route scopes remain available.' },
  { name: 'ATTESTOR_SHADOW_MUTATION_IDEMPOTENCY_PROOF', minStage: 'live-shadow', description: 'Shadow POST/PATCH mutation Idempotency-Key replay/conflict behavior was verified across retries and shared control-plane storage.' },
  ...
  { name: 'ATTESTOR_CORS_CSRF_DEPLOYMENT_PROOF', minStage: 'live-shadow', description: 'Staging browser deployment proves exact credentialed CORS origins and cookie mutation CSRF/Origin/Fetch Metadata negative behavior.' },
  { name: 'ATTESTOR_CONSEQUENCE_RETRY_ATTEMPT_LEDGER_PROOF', minStage: 'live-shadow', description: 'Consequence-admission retry-attempt records were verified through the shared atomic store across API replicas with tenant-scoped RLS posture evidence.' },

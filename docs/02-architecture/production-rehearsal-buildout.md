...
| In progress | 0 |
| Not started | 0 |
| Current posture | Step 10 is complete: the `gke-production-rehearsal` target now has a production-promotion candidate packager that consumes the filled manifest and Step 05 through Step 09 evidence artifacts, records command plan and workflow run ids, validates the production-readiness packet state, computes artifact digests, writes a tarball bundle, and signs a local Ed25519 attestation over the final archive digest. Passing this command means the named target has a signed/attested candidate evidence bundle with an explicit go/no-go verdict; it is still not market validation, not a hosted public SaaS launch, not a blanket production guarantee, and not a substitute for independent security/compliance approval. |
...
| 05 | complete | Prove external substrate readiness | `scripts/probe/probe-production-rehearsal-substrates.ts`, `tests/production-rehearsal-substrate-probe.test.ts`, `docs/08-deployment/production-rehearsal-manifest.example.json`, `docs/08-deployment/production-rehearsal-manifest.md`, `docs/08-deployment/production-rehearsal-targets/gke-production-rehearsal.json`, `package.json` | Adds `npm run probe:production-rehearsal-substrates`, a fail-closed target probe for shared PostgreSQL/Redis posture, required environment inputs, External Secrets, Gateway, HTTPRoute, cert-manager Certificate readiness, DNS target resolution, API/worker health and readiness URLs, and observability receivers. The command exits non-zero unless every substrate check passes for the named target and records non-claims in the generated evidence. |
| 06 | complete | Rehearse core fail-closed consequence behavior | `scripts/rehearse/rehearse-production-consequence-behavior.ts`, `tests/production-rehearsal-consequence-behavior.test.ts`, `docs/08-deployment/production-rehearsal-manifest.example.json`, `docs/08-deployment/production-rehearsal-manifest.md`, `package.json` | Adds `npm run rehearse:production-consequence`, a fail-closed rehearsal command that requires the Step 05 substrate readiness summary, `production-shared`, the shared release authority PostgreSQL input, and the `async-shared-authority-stores` target contract before exercising core behavior. The command uses shared authority stores by default, records admitted/proceed, blocked/hold, review/hold, token active/revoked/consumed, evidence-pack export, reviewer queue claim/release/dual approval, downstream gate behavior, and non-claims in the generated evidence. |
| 07 | complete | Rehearse queue, worker, and async recovery | `scripts/rehearse/rehearse-production-async-recovery.ts`, `tests/production-rehearsal-async-recovery.test.ts`, `src/service/async-pipeline.ts`, `src/service/async-tenant-execution.ts`, `src/service/async-weighted-dispatch.ts`, `docs/08-deployment/production-rehearsal-manifest.example.json`, `docs/08-deployment/production-rehearsal-manifest.md`, `package.json` | Adds `npm run rehearse:production-async-recovery`, a fail-closed rehearsal command that requires Step 05 and Step 06 evidence before exercising Redis/BullMQ behavior. The command records Redis `noeviction` posture, retry/backoff and stalled-job guardrail configuration, worker drain/restart, transient retry recovery, terminal failure DLQ visibility, fail-quick submission when Redis is unavailable, and Redis-backed tenant coordination. The async producer path now fails quickly when Redis is unavailable, while worker connections keep BullMQ-compatible retry posture; production-shared tenant coordination no longer silently falls back to memory when shared Redis coordination is required. |
| 08 | complete | Rehearse backup, restore, and DR | `scripts/rehearse/rehearse-production-backup-restore-dr.ts`, `tests/production-rehearsal-backup-restore-dr.test.ts`, `docs/08-deployment/production-rehearsal-manifest.example.json`, `docs/08-deployment/production-rehearsal-manifest.md`, `package.json` | Adds `npm run rehearse:production-backup-restore-dr`, a fail-closed rehearsal command that requires Step 05, Step 06, Step 07, explicit PostgreSQL PITR evidence, separated replacement PostgreSQL/Redis inputs, and replacement API/worker readiness URLs before running backup/restore behavior. The command runs the existing control-plane backup and restore commands, refuses same-source replacement URLs, validates replacement release-authority/control-plane/billing PostgreSQL state, checks Redis `noeviction` plus AOF/RDB durability posture, and runs a post-restore consequence-admission allow/block probe. |
| 09 | complete | Rehearse observability, alerting, and operator runbooks | `scripts/rehearse/rehearse-production-observability-alerting.ts`, `tests/production-rehearsal-observability-alerting.test.ts`, `docs/08-deployment/production-rehearsal-operator-runbook.md`, `ops/observability/grafana/dashboards/attestor-overview.json`, `docs/08-deployment/production-rehearsal-manifest.example.json`, `docs/08-deployment/production-rehearsal-manifest.md`, `package.json` | Adds `npm run rehearse:production-observability-alerting`, a fail-closed rehearsal command that requires Step 05, Step 06, Step 07, Step 08, explicit observability receiver URLs, explicit API health/readiness URLs, an explicit dashboard URL, and an operator runbook before core behavior runs. The command reuses the existing observability receiver and alert-routing probes, verifies runtime/shared-authority truth from health/readiness, checks the Grafana dashboard exposes `attestor_runtime_profile_info`, and verifies runbook stop conditions for runtime, shared authority, observability, alerting, and DR prerequisites. |
| 10 | complete | Package the v0.2 production-promotion candidate evidence bundle | `scripts/ops/package-production-promotion-candidate.ts`, `tests/production-rehearsal-promotion-bundle.test.ts`, `docs/08-deployment/production-rehearsal-manifest.example.json`, `docs/08-deployment/production-rehearsal-manifest.md`, `package.json` | Adds `npm run package:production-promotion-candidate`, a fail-closed packager that consumes a filled production rehearsal manifest plus prerequisite evidence artifacts, refuses placeholder workflow runs or pending evidence, verifies the production-readiness packet is ready for environment promotion, computes artifact digests, writes the production-promotion candidate archive, and signs a local Ed25519 attestation over the archive digest. A no-go bundle may still be written for operator evidence, but the command exits non-zero unless the explicit verdict is `go` and all gates pass. |
...
The production rehearsal buildout is complete at the repository level. The next work is to run the filled manifest against a real named target, review the signed production-promotion candidate evidence bundle, and only then decide whether that target receives an operator-approved production promotion.

...
| F-5.7 HA shared PKI | partial | high | `production-shared` now implicitly requires an explicit shared PKI path and shared-path attestation. Preflight reports `pkiReady=false` and avoids default local PKI persistence when the shared boundary is missing. File-backed local locking is still not a distributed lock, and external KMS/HSM remains a production promotion blocker. |

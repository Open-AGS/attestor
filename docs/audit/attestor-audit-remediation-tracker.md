...
| F11-SC-4 single OpenAI provider / provider registry contract | `partial` | `src/api/openai.ts` still uses OpenAI directly, but now records provider-returned model observations and drift warnings; `src/api/llm-provider-registry.ts` records provider inventory and fail-closed route evaluation. | Multi-provider live client selection, failover, timeout/budget enforcement, and smoke proof remain future work before resilience claims. |

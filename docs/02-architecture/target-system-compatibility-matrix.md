...
The customer-facing integration should start from a protected action, not from
manual admission-field assembly. A target-family recipe can feed the same
single consequence engine through several deployment shapes:

```text
target action
  -> hosted proxy / SDK wrapper / MCP tool wrapper / Envoy-style PEP
  -> recipe match and context extraction
  -> evidence resolver
  -> Attestor admission
  -> downstream call only when the customer-owned gate allows it
  -> receipt and replay evidence
```

The recipe layer may extract observed facts such as target system, action name,
amount, resource reference, idempotency key, actor, tenant, approval reference,
and downstream receipt reference. It may infer consequence class or authority
delta, but inferred fields must remain separate from observed fields and flow
through the [Evidence State Model](evidence-state-model.md) before any policy
candidate or rollout packet uses them.

This pattern keeps the buyer path simple:

```text
connect one target
observe action traffic
choose one high-risk action
bind evidence and approval refs
start review-required
move to canary-enforce only after customer PEP proof
```

It does not create a second product identity. It is an adapter and deployment
pattern inside the existing Attestor consequence boundary.

### Action Recipe Manifest Shape

Future recipes should be machine-readable so the same contract can generate
shadow events, admission requests, review fields, tests, and deployment
instructions:

```text
id
targetSystem
match method/path/tool/workflow operation
consequenceClass
extract observed fields
evidence resolver refs
policy ref
idempotency/replay refs
receipt ref
raw material policy
deployment modes supported
```

The manifest is not a native connector claim. A recipe becomes runtime-wired
only when the corresponding proxy, wrapper, MCP server, Envoy/ext_authz
configuration, SDK helper, or customer PEP is implemented and tested. It
becomes live-proofed only when the target deployment proves no-bypass,
fail-closed behavior, replay/idempotency behavior, credential boundaries, and
receipt handling in the customer or staging environment.

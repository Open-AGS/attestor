...
| `OPS-SWEEP-13` | Onboarding + admission routes deep audit across generic admission, action-surface onboarding packet, and policy-foundry hosted onboarding workflow/session routes. | `58ca89c1e7d16c110bb65d71bd9edf6a303c64ac` | `partial` | `docs/audit/ops-sweep-13-onboarding-admission-deep-audit.md`. | No P0/P1 surfaced. OPS-106/107/108 remain P2 hardening follow-ups; OPS-109/110 remain P3 cleanup; OPS-111 is accepted. Continue final route-surface Sweep 14. |
| `OPS-SWEEP-14` | Final route surface deep audit across pipeline verification, filing export, public-site, and core diagnostic/trust-root routes; completes the `/api/v1/**` route-layer deep-audit chain. | `a21eb1830db3cc7a70ba6a374e8bfdb7066bdb34` | `partial` | `docs/audit/ops-sweep-14-final-route-surface-deep-audit.md`. | OPS-112 `/health`/`/ready` diagnostic disclosure and OPS-113 `/verify` rate-limit gap remain P1 route-side blockers before Phase 1 exit. After remediation, move route work to live probes and non-route Phase 1 work. |
| `OPS-SWEEP-15` | Branch protection / CI governance deep audit. 13 governance artifacts audited (4 policy docs + 9 workflows); branch policy and CODEOWNERS declared, workflow actions SHA-pinned, permissions minimized, PR contract mechanically enforced, supply-chain gates layered, CodeQL security-extended, release provenance attested for evaluation artifacts, external-live env-gated, and production rehearsal manual-only. First non-route sweep after the `/api/v1/**` route-layer deep-audit chain closed. | `585b85445a2a73f555cd05d469392ead9abe1f19` | `partial` | `docs/audit/ops-sweep-15-branch-protection-deep-audit.md`. | OPS-121 required signed-commit protection and OPS-122 required review protection remain P1 release-provenance blockers before Phase 1 exit. Closure requires one branch-governance workflow extension plus operator GitHub-settings action and negative merge proof. |
| `OPS-SWEEP-16` | Demo safety + redaction deep audit. 10 in-scope artifacts mapped (demo CLI, pure-code examples, render scripts, runtime redactor, architectural policy, committed evidence, and demo fixture); demo CLI shadow-only no-claim banner, synthetic fixture, public-key-only committed evidence inventory, and Stripe-focused redaction are repo-proven; missing AWS/GCP/GitHub/JWT/private-key/Slack/Anthropic/OpenAI provider redaction patterns and public-artifact scan are localized. | `065814d1a8c1b46f9fa36ed36bb0086f5847fd28` | `partial` | `docs/audit/ops-sweep-16-demo-safety-redaction-deep-audit.md`. | OPS-129 provider redaction, OPS-132 public artifact scan, and OPS-133 demo path basedir constraint remain public-demo blockers. These supersede `B-028` and `B-025` with concrete remediation paths. |
| `OPS-SWEEP-17` | Test adequacy meta-audit. 37 P0/P1 finding-index rows mapped to locking test evidence; 19 repo-locked, 13 live-proof-only, and 5 open or needing test contracts. Regression-lock infrastructure is repo-proven for the route surface and ops sweeps 03/04; gaps are localized to meta-tooling: no automated guard, no `Locking test:` schema marker, no future test-contract path for several live-proof findings, and no renderer-output test for OPS-25. | `5ebab103648967db48652949eacff4dd794ec2ce` | `partial` | `docs/audit/ops-sweep-17-test-adequacy-meta-audit.md`. | Promote OPS-136 and OPS-139 closure together to fully close the `Test adequacy map` finding; optional same-PR cleanup can close OPS-137/138/140. |

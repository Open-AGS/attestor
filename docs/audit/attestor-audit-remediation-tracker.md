...
| ID | Current status | Evidence / PR | Remaining action |
|---|---|---|---|
| F2-AG-1 customer-gate honor-system | `partial` | `docs/audit/f2-customer-gate-enforcement-validation.md`; signed bearer and release-enforcement customer-gate verifiers in `src/consequence-admission/customer-gate.ts` | Customer-gate helper now has a signed bearer release-token compatibility verifier and a protected release-enforcement verifier-consumer path that requires sender constraint, online introspection, replay consumption, tenant/audience binding, and admission proof binding. Generic consequence-to-token binding and customer runtime activation remain future work. |
...
| F4-LLM06-A customer gate honor-system | `partial` | Same root as F2-AG-1 and F1-CC-2; `docs/audit/f2-customer-gate-enforcement-validation.md` | The LLM06 claim is narrowed: helper-only use remains honor-system; protected release-enforcement path exists separately and can now be consumed by the customer gate when sender constraint, online liveness, replay consumption, and proof binding are already proven. |

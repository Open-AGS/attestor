...
| F6-T6 | Compromise of runtime signer affects all tenants. | `partial` | Tenant-bound release-token claims narrow token reuse, but runtime signer and release-token verification key are still shared runtime material. Revoking that signer is runtime-wide. The tenant signer boundary contract proves the desired external signer contract and fake KMS mismatch behavior, but does not replace the runtime signer. | Wire a live per-tenant leaf signer or KMS/HSM tenant-scoped signer before claiming signer-compromise blast-radius isolation. |
...
2. Tenant-bound release-token/admission semantics for F6-T1/F6-T6. Done for token semantics and tenant signer boundary contract; live per-tenant signer isolation remains partial.

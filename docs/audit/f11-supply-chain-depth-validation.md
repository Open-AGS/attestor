...
| F11-SC-4 single OpenAI provider / provider registry contract | `partial` | Attestor-owned OpenAI calls now record response-model drift telemetry, and `src/api/llm-provider-registry.ts` records provider inventory plus fail-closed route evaluation. Multi-provider live client selection, failover, timeout/budget enforcement, and smoke proof remain backlog. |

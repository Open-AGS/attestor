...
| upgrade through workflow checkout | `POST /api/v1/account/billing/workflows/checkout` | customer | Stripe checkout session and URL are returned for a workflow tier; `Idempotency-Key` protects retries | idempotency key, workflow id, tier, consequence pack, required digests, or account/billing admin authority is missing |
...
| `POST /api/v1/account/billing/workflows/checkout` | account session with account or billing admin role | workflow checkout body plus `Idempotency-Key` | `AccountWorkflowBillingCheckoutResponse` |

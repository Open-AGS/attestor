...
| OPS-170 | P3 | `closed` | `index.ts` size and barrel surface received a focused sub-sweep before deeper behavior claims | `src/consequence-admission/index.ts` is 2440 lines with 235 `export` statements and centralizes normalization, generic envelope creation, retry-budget evaluation, feedback, and descriptors. `docs/audit/ops-170-consequence-admission-index-focused-sub-sweep.md` and `tests/consequence-admission-index-surface.test.ts` now lock the response builder, generic envelope, retry-budget evaluator, retry instruction helper, and existing behavioral test coverage. | auditability; operational boundedness | No repo-side action remains for OPS-170; future splits are allowed only when they preserve these locked contracts. |
| OPS-171 | P2 | `accepted limitation / sub-sweep backlog` | Consequence-admission family sub-sweeps are queued before family-level behavioral claims | Sweep 22 inventoried but did not behaviorally audit the `policy-foundry-*`, `shadow-*`, assurance/invariant, action-surface, customer-gate, and admission test families. `docs/audit/ops-171-consequence-admission-family-sub-sweep-queue.md` now names queue slices, trigger conditions, authority boundaries, and minimum closure evidence. | auditability; no overclaim | Run queued family sub-sweeps only when module-specific risk warrants them; do not claim family-level behavioral closure from the queue alone. |
...
| OPS-170 focused `index.ts` sub-sweep | completed by `docs/audit/ops-170-consequence-admission-index-focused-sub-sweep.md` and `tests/consequence-admission-index-surface.test.ts` | low, no runtime code change | targeted checks for response/envelope/retry branches |
| OPS-171 family sub-sweep queue | completed by `docs/audit/ops-171-consequence-admission-family-sub-sweep-queue.md`; avoids overclaiming policy-foundry/shadow/invariant behavior | low, no runtime code change | dedicated sub-sweep reports only when module-specific risk warrants |

...
| OPS-125 | P2 | `open / partial-repo` | CODEOWNERS uses a single org-wide owner for all trust-sensitive surfaces. | `.github/CODEOWNERS` maps all protected surfaces to `@AI-gateway-systems`. | release provenance; auditability | Split into per-surface teams after branch protection enforcement is enabled. |

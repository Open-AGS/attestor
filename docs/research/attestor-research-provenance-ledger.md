...
| `docs/02-architecture/policy-foundry-self-onboarding-deepening.md` | AWS IAM Access Analyzer, Google IAM Recommender, Cedar/AWS Verified Permissions, OPA decision logs/tests, Stripe Connect onboarding, Auth0 progressive profiling, Envoy/Istio/OPA external authorization, OpenFGA/Zanzibar authority modeling. | Step 01 adds the first onboarding session contract for requirements-aware blockers and digest-bound source evidence; later coverage scoring, gate planning, schema-bound registry, counterexample ledger, UI, and commercial entitlement work remain open. | Architecture research tracker and current implementation evidence; exact PR/commit mapping is pending until the self-onboarding session PR is merged. |

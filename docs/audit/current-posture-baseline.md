...
| Service/API security | 6 | partial-repo | Edge contract, rate limit, auth/MFA/session/federated auth sampled and generally strong; OPS-SWEEP-06 maps and hardens the 32-route admin surface repo-side. | `src/service/**` remains only partially audited; admin role-key deployment and public route probes still need live proof. | Continue public route authZ probes and capture admin role-key/rate-limit live evidence. |
...
| Cloud / ops readiness | 5 | not proven / needs ops proof | Docs are honest about non-claims; repo has production-readiness scaffolding, HA ops gates, PKI/TLS gates, observability remediation indexes, and PITR/Redis recovery hardening. | `ops/**`, IAM, KMS, managed observability backend proof, live PITR/Redis recovery proof, logging redaction, and billing controls are not fully assessed. | Continue ops audit with autoscaling, profiles, recording rules, and live proof capture. |
...
| `ops/**` audit gap | partial | not proven | OPS-SWEEP-01/02/03/04 are indexed; remaining autoscaling, profiles, recording rules, provider overlays, and live proof are incomplete. | Continue ops audit before live shadow. |

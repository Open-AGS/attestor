...
function testTrialKeepsSafetyMinimumsButBlocksProduction(): void {
  ...
  equal(boundary.plan, 'trial', 'Commercial boundary: trial plan is selected');
  equal(boundary.paidPlan, false, 'Commercial boundary: trial is not paid');
  equal(boundary.hostedProductionAllowed, false, 'Commercial boundary: trial has no hosted production entitlement');
  equal(boundary.productionWorkflowRequestAllowed, false, 'Commercial boundary: trial production request is blocked');
  ok(boundary.noGoReasons.includes('production-enforcement-not-in-plan'), 'Commercial boundary: trial production no-go is explicit');
...
function testStarterAllowsOnlyOneCommercialWorkflowWithoutClaimingReadiness(): void {
  ...
  equal(boundary.plan, 'starter-workflow', 'Commercial boundary: starter workflow tier is selected');
  equal(boundary.paidPlan, true, 'Commercial boundary: starter is paid');
  equal(boundary.hostedProductionAllowed, true, 'Commercial boundary: starter workflow has hosted production commercial entitlement');
  equal(boundary.productionWorkflowRequestAllowed, true, 'Commercial boundary: one starter workflow entitlement is commercially allowed');
  equal(boundary.noGoReasons.length, 0, 'Commercial boundary: starter workflow request has no commercial no-go');
...
function testEnterpriseAllowsCustomerOperatedCapabilityAsBoundaryOnly(): void {
  ...
  equal(boundary.plan, 'negotiated-deployment', 'Commercial boundary: negotiated deployment boundary is selected');
  equal(boundary.customerOperatedAllowed, true, 'Commercial boundary: negotiated deployment supports customer-operated deployment commercially');
  equal(boundary.noGoReasons.length, 0, 'Commercial boundary: negotiated customer-operated request has no commercial no-go');
...
function testSafetyMinimumCannotBePaywalledEvenOnNegotiatedDeployment(): void {
  const boundary = createPolicyFoundryCommercialBoundary({
...
function testDescriptorDocsAndPackageSurfaceStayAligned(): void {
  ...
  ok(descriptor.plans.includes('negotiated-deployment'), 'Commercial boundary descriptor: negotiated deployment boundary is named');
...
try {
testTrialKeepsSafetyMinimumsButBlocksProduction();
  ...
  testSafetyMinimumCannotBePaywalledEvenOnNegotiatedDeployment();

...
- OpenTelemetry semantic conventions define common span, metric, log, and resource attributes so telemetry can be correlated across tools; Attestor uses those names where useful but keeps private customer context out of exported signal attributes: [OpenTelemetry semantic conventions](https://opentelemetry.io/docs/specs/semconv/)
- OpenTelemetry attribute requirement levels explicitly account for security and cardinality, and high-cardinality metric attributes belong behind opt-in behavior rather than default collection. Attestor request metrics therefore use method, route label, status, and trace-context state instead of raw customer identifiers: [OpenTelemetry attribute requirement levels](https://opentelemetry.io/docs/specs/semconv/general/attribute-requirement-level/)
- OWASP logging guidance requires excluding secrets, sensitive personal data, tokens, connection strings, encryption keys, and other sensitive material from logs. Attestor observability exports therefore use presence booleans and trace ids instead of raw tenant ids, account ids, paths, IPs, user agents, payloads, or provider bodies: [OWASP Logging Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html)
- NIST SP 800-61 Rev. 3 frames incident response as a cybersecurity risk-management practice that should improve preparation, detection, response, and recovery. Attestor incident evidence should therefore be a prepared packet shape with timeline, evidence refs, action owners, and privacy boundaries before a live incident: [NIST SP 800-61 Rev. 3](https://csrc.nist.gov/pubs/sp/800/61/r3/final)
- Google SRE postmortem culture treats postmortems as written records of impact, mitigation or resolution actions, root cause, and follow-up actions. Attestor incident packets keep that evidence shape while excluding raw customer data: [Google SRE postmortem culture](https://sre.google/sre-book/postmortem-culture/)

## Step List

| Step | Status | Deliverable | Evidence | Notes |
|---|---|---|---|---|
| 01 | complete | Hosted API Authorization Matrix | `src/service/hosted/hosted-api-authorization-matrix.ts`, `tests/hosted-api-authorization-matrix.test.ts`, `src/service/http/routes/pipeline-async-routes.ts`, `docs/01-overview/hosted-journey-contract.md`, `package.json` | Adds a machine-readable route authorization matrix for public metadata, credential challenge, account plane, tenant runtime, signed webhooks, shadow control, and operator admin routes. Also fixes the confirmed async job status BOLA edge by requiring the job tenant to match the current tenant before returning status. |
| 02 | complete | Sensitive Business Flow Abuse Guard | `src/service/hosted/hosted-sensitive-business-flow-abuse-guard.ts`, `tests/hosted-sensitive-business-flow-abuse-guard.test.ts`, `src/service/http/routes/account-routes.ts`, `package.json` | Adds a machine-readable abuse-control profile for checkout, portal, API-key issue/rotate/status/revoke, tenant admin, signup/bootstrap, auth challenges, tenant execution, provider webhooks, and release-bound export. Also wires SAML and OIDC login initiation through the shared auth abuse budget. |
| 03 | complete | Webhook And Async Reconciliation Hardening | `src/service/hosted/hosted-webhook-async-reconciliation-hardening.ts`, `tests/hosted-webhook-async-reconciliation-hardening.test.ts`, `src/service/account/account-store.ts`, `src/service/control-plane-store.ts`, `src/service/application/stripe-webhook-billing-processor.ts`, `package.json` | Adds a machine-readable reconciliation profile for Stripe ingress, billing convergence, email provider webhooks, tenant async execution, and dead-letter recovery. Also adds provider-event ordering guards so older Stripe subscription and invoice events are finalized as ignored instead of overwriting fresher state. |
| 04 | complete | LLM/Agent Tool-Use Boundary Guard | `src/service/hosted/hosted-llm-agent-tool-boundary-guard.ts`, `tests/hosted-llm-agent-tool-boundary-guard.test.ts`, `docs/api/attestor-action-authorization.openapi.json`, `tests/hosted-action-authorization-openapi.test.ts`, `package.json` | Adds a machine-readable boundary profile across model-safe admission feedback, agent retry authority, shadow recommendation reads, protected adapter execution, and proof/dashboard/problem-detail surfaces. Also extends the OpenAPI contract with model-safe feedback and explicit no-tool-authority/no-unsafe-retry boundary flags. |
| 05 | complete | Production Runtime Health Contract | `src/service/hosted/hosted-production-runtime-health-contract.ts`, `tests/hosted-production-runtime-health-contract.test.ts`, `src/service/http/routes/core-routes.ts`, `src/service/async/worker.ts`, `package.json` | Adds a machine-readable health contract across API process health, API dependency readiness, worker health/readiness, async queue runtime, storage/release authority readiness, webhook ingress readiness, and degraded-mode visibility. Also makes API and worker probe responses explicitly no-store at the route/server level. |
| 06 | complete | Release Provenance And SLSA Alignment | `src/service/hosted/hosted-release-provenance-slsa-alignment.ts`, `tests/hosted-release-provenance-slsa-alignment.test.ts`, `.github/workflows/release-provenance.yml`, `docs/08-deployment/artifact-attestation-plan.md`, `docs/01-overview/hosted-journey-contract.md`, `docs/08-deployment/production-readiness.md`, `package.json` | Adds a machine-readable release provenance/SLSA alignment profile across workflow identity, artifact subject/digest verification, SBOM/dependency evidence, package-surface gates, proof/evidence packet binding, and production non-claim boundaries. Also publishes separate build provenance and CycloneDX SBOM attestations for the release evaluation archive. |
| 07 | complete | Observability Privacy And Incident Evidence | `src/service/hosted/hosted-observability-privacy-incident-evidence.ts`, `tests/hosted-observability-privacy-incident-evidence.test.ts`, `src/service/observability.ts`, `src/service/request-observability-middleware.ts`, `docs/01-overview/hosted-journey-contract.md`, `docs/08-deployment/production-readiness.md`, `package.json` | Adds a machine-readable observability privacy and incident-evidence profile across request traces/metrics/logs, receiver probes, alert routing, incident packet shape, dashboard runtime truth, and rehearsal evidence. Also changes request observability exports to omit raw URLs, paths, tenant ids, account ids, IPs, and user agents. |
...
Steps 01-07 are complete in code and test coverage. Steps 08-09 remain implementation work. Step 10 remains blocked until a working deployment target is available.

## Step 02 Evidence

The sensitive business flow guard is repo-side hardening, not a production rollout claim.

- `src/service/hosted/hosted-sensitive-business-flow-abuse-guard.ts` declares the sensitive flow profile across auth challenges, first-user bootstrap, API-key lifecycle, Stripe Checkout, Stripe Portal, tenant execution, operator admin mutations, signed provider webhooks, and release-bound filing export.
...
- `src/service/hosted/hosted-webhook-async-reconciliation-hardening.ts` declares the reconciliation profile across Stripe signed ingress, Stripe billing convergence, email provider callbacks, tenant async execution, and dead-letter recovery.
...
- `src/service/account/account-store.ts` and `src/service/control-plane-store.ts` now persist provider-created timestamps for subscription and invoice event lanes and reject stale provider events before they can regress account or billing state.
...
- `src/service/hosted/hosted-llm-agent-tool-boundary-guard.ts` declares the boundary profile across model-safe feedback, retry authority, shadow recommendation reads, protected adapter execution, and proof/dashboard/problem-detail surfaces.
...
- `src/service/hosted/hosted-production-runtime-health-contract.ts` declares the health contract across API liveness/startup diagnostics, API readiness, worker readiness, async queue runtime, storage/release authority readiness, webhook ingress readiness, and degraded-mode visibility.
...
- `src/service/async/worker.ts` now sets `cache-control: no-store` directly on worker `/health` and `/ready` probe responses.
...
- `src/service/hosted/hosted-release-provenance-slsa-alignment.ts` declares the release provenance profile across release workflow identity, artifact subject/digest verification, SBOM/dependency evidence, package-surface gates, proof/evidence packet binding, and production non-claim boundaries.
...
The observability privacy and incident evidence profile is repo-side hardening, not a production rollout claim.

- `src/service/hosted/hosted-observability-privacy-incident-evidence.ts` declares the observability profile across privacy-safe request traces/metrics/logs, OTLP receiver boundaries, alert routing context, incident timeline/evidence packets, dashboard runtime truth, and production rehearsal evidence.
- `tests/hosted-observability-privacy-incident-evidence.test.ts` verifies source evidence, validation evidence, required controls, external research anchors, package-runner exposure, docs exposure, no-secret contract output, incident packet shape, and no-raw-customer-data telemetry projection.
- `src/service/observability.ts` now writes local JSONL request logs and OTLP request logs through the same privacy-safe projection: route labels are normalized, raw paths are omitted, tenant/account context is reduced to presence booleans, and IP/user-agent values are not exported.
- `src/service/request-observability-middleware.ts` now uses `__unmatched__` / `__unrouted__` labels when a route template is unavailable, avoiding raw path labels for 404 or unbound request paths.
- Production observability is still separate from this repo-side guard: a working deployment target, managed observability backend credentials, receiver probes, alert receiver delivery verification, webhook smoke tests, and hosted product smoke tests remain Step 10 work.

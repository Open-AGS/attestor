...
| 12 | complete | Build the reference Node and Hono middleware PEP | `src/release-enforcement-plane/middleware.ts`, `tests/release-enforcement-plane-middleware.test.ts` | A reusable middleware path now makes `no release authorization -> no consequence` easy to adopt on ordinary HTTP consequence surfaces. The reference PEP includes a shared HTTP evaluation core, Hono `createMiddleware()` adapter, Node `IncomingMessage`/`ServerResponse` adapter, fail-closed deny responses, default `GET`/`HEAD` protection, explicit protected-method configuration only with method-coverage proof for intentional opt-outs, handler context/result injection after allow, trusted server-side binding resolvers by default, and explicit `trusted-upstream` header mode only with upstream proof for pre-bound gateway integrations. |
...
| 15 | complete | Build the communication-send enforcement gateway | `src/release-enforcement-plane/communication-send.ts`, `tests/release-enforcement-plane-communication-send.test.ts` | Email, memo, webhook, chat, SMS, and internal-message boundaries now have a dedicated enforcement adapter that canonicalizes the exact outbound communication request, binds output and consequence hashes to the target endpoint and HTTP send transport, labels attachment metadata as `declaredAttachments`, requires `sha256:<64 lowercase hex>` digest references, requires DPoP-bound or HTTP-message-signature sender-constrained presentation, performs online introspection and token-use consumption by default, and fails closed on missing authorization, payload/target mismatch, bearer downgrade, revocation, replay, or control-plane outage. Attachment bytes are not semantically verified unless the embedding adapter verifies them before calling this gateway. |
| 16 | complete | Build the action-dispatch enforcement gateway | `src/release-enforcement-plane/action-dispatch.ts`, `tests/release-enforcement-plane-action-dispatch.test.ts` | Tool calls, workflow steps, HTTP calls, job starts, and async dispatch boundaries now go through a dedicated action gateway that canonicalizes the exact downstream action request, binds output and consequence hashes to the target workflow, endpoint, or queue and HTTP dispatch transport, labels precondition metadata as `declaredPreconditions`, requires `sha256:<64 lowercase hex>` digest references when precondition digests are present, requires DPoP/mTLS/SPIFFE sender-constrained presentation, performs online introspection and token-use consumption by default, and fails closed on missing authorization, payload/target mismatch, bearer downgrade, revocation, replay, or control-plane outage. Preconditions are not semantically verified unless the embedding adapter verifies the underlying evidence before calling this gateway. |

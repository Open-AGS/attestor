...
| Live proof separation | runtime readiness; operational boundedness | `docs/audit/live-proof-register.md`; `scripts/check-ops-live-shadow-readiness.mjs`; OPS-SWEEP-07 stage-aware proof gate alignment; OPS-SWEEP-08 webhook provider proof gates; OPS-SWEEP-09 account callback/audit proof gates. | HTTPS, IAM, KMS, ClusterSecretStore, WAF, replay/introspection, customer PEP, KEDA Redis TLS, KEDA Prometheus auth, admin role-key/rate-limit behavior, webhook provider endpoint/fake-signature proof, federated callback rate-limit proof, account mutation audit-chain proof, and shared auth-abuse-store proof. | Capture live proof artifacts without raw customer data; keep register entries aligned with the gate. |
...
| Service/API boundary | customer authority; tenant isolation; auditability | Edge contract, auth/MFA/session/federated auth sampled; OPS-SWEEP-06 maps the 32-route admin surface; OPS-SWEEP-08 maps and hardens the 3 inbound webhook signature surfaces; OPS-SWEEP-09 maps the 49-route account surface and adds federated callback rate limiting plus account-session mutation audit. | Live route probes, customer PEP integration, role-scoped admin key deployment, legacy superuser-key rotation, admin/webhook/account callback rate-limit abuse proof, provider webhook endpoint proof, email webhook replay-store proof, and account mutation audit-chain proof. | Continue remaining public route authZ slices (`pipeline-*`, release-review, release-policy-control, shadow); keep admin read-audit, response key-material, Mailgun body-signature, account idempotency, account JSON parsing, and password-policy limitations visible. |

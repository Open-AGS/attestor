...
export const GOLDEN_REFUND_RUNTIME_SMOKE_VERSION =
  'attestor.golden-refund-runtime-smoke.v1';

export interface GoldenRefundRuntimeSmokeScenarioResult {
...
export interface GoldenRefundRuntimeSmokeResult {
  readonly version: typeof GOLDEN_REFUND_RUNTIME_SMOKE_VERSION;
  readonly step: 'G05';
...
export interface GoldenRefundRuntimeSmokeDescriptor {
  readonly version: typeof GOLDEN_REFUND_RUNTIME_SMOKE_VERSION;
  readonly step: 'G05';
  readonly sourceFixtureSuiteVersion: typeof GOLDEN_REFUND_SHADOW_FIXTURES_VERSION;
  readonly sourcePolicyFoundryProjectionVersion:
...
const GENERATED_AT = '2026-05-19T08:00:00.000Z';
...
function minute(index: number, seconds: number): string {
  return `2026-05-19T08:${String(index).padStart(2, '0')}:${String(seconds).padStart(2, '0')}.000Z`;
}

function assertProjectionMatchesSuite(
  suite: GoldenRefundShadowFixtureSuite,
  projection: GoldenRefundPolicyFoundryProjection,
): void {
  if (projection.sourceFixtureSuiteDigest !== suite.digest) {
    throw new Error(
      'Golden refund runtime smoke projection must be derived from the fixture suite.',
    );
  }
  if (projection.autoEnforce || projection.activatesEnforcement || projection.productionReady) {
    throw new Error('Golden refund runtime smoke requires a review-only projection.');
  }
}
...
function runScenarioSmoke(
  ...
  if (!fixture) {
    throw new Error('Golden refund runtime smoke requires a fixture for each scenario.');
  }
  return runShadowRuntimeFixtureReplaySmoke({
    fixtureId: fixture.fixtureId,
    fixtureRefDigest: fixture.digest,
    event: fixture.event,
    sourcePartitionDigest: digestFor('golden-refund-source-partition', suite.digest),
    traceContextDigest: digestFor('golden-refund-trace-context', {
      scenario: fixture.scenario,
      eventDigest: fixture.event.digest,
    }),
    sourceHistoryRefDigest: digestFor('golden-refund-source-history', suite.digest),
    ...
    workerRefDigest: digestFor('golden-refund-worker', 'fixture-only-worker'),
    dispatcherRunDigest: digestFor('golden-refund-dispatcher', {
      scenario: fixture.scenario,
      sequence: index,
    }),
    observerRefDigest: digestFor('golden-refund-observer', 'fixture-only-observer'),
    evaluatorRefDigest: digestFor('golden-refund-evaluator', 'fixture-only-evaluator'),
    scopeDigest: digestFor('golden-refund-scope', 'shadow-only-runtime-smoke'),
  });
}

function scenarioResult(
  smoke: ShadowRuntimeFixtureReplaySmokeResult,
): GoldenRefundRuntimeSmokeScenarioResult {
  return Object.freeze({
    scenario: smoke.fixtureId.replace(/^golden-refund:/u, ''),
...
export function runGoldenRefundRuntimeSmoke(
  suite: GoldenRefundShadowFixtureSuite = createGoldenRefundShadowFixtureSuite(),
  projection: GoldenRefundPolicyFoundryProjection =
    createGoldenRefundPolicyFoundryProjection(suite),
): GoldenRefundRuntimeSmokeResult {
  ...
  const payload = {
    version: GOLDEN_REFUND_RUNTIME_SMOKE_VERSION,
    step: 'G05',
...
export function goldenRefundRuntimeSmokeDescriptor(): GoldenRefundRuntimeSmokeDescriptor {
  return Object.freeze({
    version: GOLDEN_REFUND_RUNTIME_SMOKE_VERSION,
    step: 'G05',
